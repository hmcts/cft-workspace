...
Adding entries to `docs/microservices.json` (e.g. a new service's group/nodes) does not, by itself,
update the per-group Low Level Design pages under `platops/cnp-api-docs/docs/lld/*.html`. Those are
committed HTML artifacts generated by `docs/generate-llds.js`, and no CI workflow in `cnp-api-docs`
runs that script — only the C4 diagrams have a workflow (`update-images.yml`) that regenerates and
commits them on every PR. A `microservices.json` change needs `node docs/generate-llds.js` run and
committed by hand if the corresponding LLD page should reflect it.
