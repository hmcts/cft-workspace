...
- A handful of files under `docs/specs/` are empty placeholders (0 bytes) rather than valid JSON. A script that loops over `*.json` and pipes each straight into `jq` should skip empty files first — otherwise they fail silently or abort the loop instead of just being reported as unparseable.
