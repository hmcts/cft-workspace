...
**Trap**: if an application compresses assets itself at request time (for example Express's `compression()` middleware) rather than precompressing at build time, the response has no `Content-Length` — it can only be sent `Transfer-Encoding: chunked`, because the compressed size isn't known until the stream finishes. Front Door cannot cache a chunked response, and a route/asset marked cacheable (`Cache-Control: public, ..., immutable`) that arrives chunked and compressed stalls indefinitely, returning zero bytes until the client times out. Requests without `Accept-Encoding` (plain `curl`, most health checks) are unaffected, since they get the uncompressed, length-declared response — which makes this failure mode look like intermittent slowness rather than a hard break. Precompressing at build time (so the compressed asset has a fixed `Content-Length`) avoids the problem entirely.
