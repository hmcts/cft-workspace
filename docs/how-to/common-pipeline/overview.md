...
The `dependencyCheckAggregate` task itself runs wrapped in a `warnError` block, so a `FAILURE: Build failed with an exception` line against that task in the console log does not, by itself, fail the overall build — the pipeline continues past it. Treat a `dependencyCheckAggregate` failure in the log as a warning to investigate, not proof of what actually broke a red build; keep reading for the step that genuinely failed it.
