...
- Team-specific tooling under `teams/<team>/` is packaged as a Claude Code plugin and has no Codex adapter. The files are ordinary Markdown and shell scripts — read `teams/<team>/skills/<name>/SKILL.md` directly and follow it, but expect no `$<team>:<name>` invocation and no `${CLAUDE_PLUGIN_ROOT}` expansion; substitute `teams/<team>` for that variable yourself.
