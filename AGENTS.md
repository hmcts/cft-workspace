...
- Team-specific tooling under `apps/<product>/.claude/` is packaged as a Claude Code plugin and has no Codex adapter. The files are ordinary Markdown and shell scripts — read `apps/<product>/.claude/skills/<name>/SKILL.md` directly and follow it, but expect no `$<product>:<name>` invocation and no `${CLAUDE_PLUGIN_ROOT}` expansion; substitute `apps/<product>/.claude` for that variable yourself.
