...
- `apps/et/et-sya-api` — Spring Boot Java API for the self-represented claimant portal (SYA = Submit Your Appeal); creates and manages CCD cases on behalf of citizens
...
- `apps/et/et-hearings-api` — Spring Boot Java API (port 4560) integrating with HMC via Azure Service Bus topic to process hearing updates; publishes `et-hearings-api.json` spec
...
- `am`: role-to-access-profile definitions in CCD imply AM-backed RBAC; `am-role-assignment-service` not called directly but consumed via CCD's RBAC evaluation.
...
- `cftlib`: `com.github.hmcts.rse-cft-lib` in `et-ccd-callbacks`; provides `bootWithCCD` task and embedded CCD for local dev.
...
- Docmosis Tornado (not a taxonomy token): third-party document rendering engine; configured via `TORNADO_ACCESS_KEY` / `TORNADO_URL` in `et-ccd-callbacks` and `et-sya-api`.
- Azure Service Bus (not a taxonomy token): `et-hearings-api` subscribes to HMC topic `hmc-to-hearings-api`; connection configured via `HMC_HEARINGS_TOPIC_*` environment variables.
- ACAS API (not a taxonomy token): `et-sya-api` calls `${ACAS_BASE_URL}` with an API key to fetch ACAS conciliation certificates during ET1 submission.
...
- The `hmcts.ccd.sdk` Gradle plugin appears in `et-sya-api` and `et-hearings-api` builds but is used as a bean scanning mechanism, not to emit CCD definitions — definitions all live as JSON in `et-ccd-callbacks`.
- The legacy PET stack (et-pet-*) is Ruby/Rails and shares a Postgres+Redis cluster; `et-pet-ccd-export` is the bridge to CCD via Sidekiq.
