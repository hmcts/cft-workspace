...
- `apps/wa/wa-ccd-definitions` — JSON CCD case-type definitions for the WA test/demo case type (`appeal`), including WorkBasket and SearchInput/Result configurations; used for local testing and onboarding reference
...
`wa-task-monitor` runs scheduled Camunda polls to detect tasks in an `Unconfigured` state (tasks that missed their initial configuration callback) and re-triggers configuration. It also runs periodic maintenance jobs: termination, reconfiguration (after `reconfigure_request_time_hours`), and Camunda clean-up in non-production environments.
...
- `idam`: all Java services validate bearer tokens against IDAM JWKS (`/o/jwks`); `wa-reporting-frontend` uses OIDC. System user credentials used by `wa-task-management-api` and `wa-task-monitor` to perform privileged operations.
...
- `flyway`: `wa-task-management-api`, `wa-workflow-api`, and `wa-case-event-handler` each manage their own PostgreSQL schema with Flyway migrations; each service owns a separate database.
...
- `wa-task-management-api` supports a **read replica** datasource (`datasource-replica`) for task search queries, configured via `POSTGRES_REPLICA_HOST`/`POSTGRES_REPLICA_PORT`. Logical replication must be enabled separately (`SPRING_PROFILES_ACTIVE=replica`).
- All three Java API services integrate **LaunchDarkly** feature flags (`launchDarkly.sdkKey`) for runtime feature toggling.
- Allowed jurisdictions and case types are configured as `config.allowedJurisdictions` and `config.allowedCaseTypes` in `wa-task-management-api`, covering IA, WA, SSCS, Civil, Public Law, Private Law, Employment, and ST CIC.
...
- `wa-post-deployment-ft-tests` runs hourly via `Jenkinsfile_nightly` cron (`0 * * * *`) and uses `withNightlyPipeline()` rather than a standard deployment pipeline.
