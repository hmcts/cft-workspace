...
confluence:
  ...
  - id: "272434866"
    ...
    last_modified: "2026-09-12T18:18:47.000Z"
    space: "RRFM"
  - id: "277949014"
    title: "HLD CCD - 5.0"
...
| A redeploy of the decentralised service can surface as a plain 500 to the end user | If the service's ingress is briefly absent while a pod is mid-redeploy, the ingress controller answers with its own plain-text 404 ("page not found") instead of a connection error. CCD's Feign client reports this as `FeignException$NotFound`, and the data-store's exception handler turns that into a generic 500 for the caller — indistinguishable at the browser from a real fault in the service. Before assuming a data or code bug, check whether the target case type's pod/ingress was mid-redeploy at the failure timestamp. |
