...
confluence:
  - id: "278762978"
    ...
    last_modified: "2026-02-17T15:11:28.000Z"
    space: "AM"
  - id: "278763127"
    title: "HLD - Case Group Access v1.1"
    last_modified: "2025-12-16T16:16:04.000Z"
    space: "AM"
  - id: "277949690"
    title: "Professional (Group) Access Onboarding"
    last_modified: "2026-06-19T15:21:47.000Z"
    space: "RCCD"
  - id: "277949114"
  ...
  - id: "277948570"
    ...
    last_modified: "2026-05-19T15:48:47.000Z"
    space: "RCCD"
  - id: "277948991"
    title: "Case Group Access Scope of Delivery"
    last_modified: "2022-10-20T11:14:58.000Z"
    space: "RCCD"
...
sources_sha:
  ...
  "rd-professional-api:src/main/java/uk/gov/hmcts/reform/professionalapi/util/ProfileOrgTypeUtility.java": "53085300965ee773e0fa62fe633b374e8658f2d0"
  "rd-professional-api:src/main/java/uk/gov/hmcts/reform/professionalapi/util/OrganisationProfileIdConstants.java": "53085300965ee773e0fa62fe633b374e8658f2d0"
...
`uk.gov.hmcts.ccd.sdk.type.CaseAccessGroup` is `@ComplexType(generate = false)` because the definition store predefines the type. The JSON property name must be exactly `CaseAccessGroups` to match what data store writes.

Four details of the shipped design are worth knowing because they encode gotchas documented elsewhere on this page:

- **The role names the group, not the other way round.** A `CCDAccessGroup` constant carries the whole `AccessType` row plus the group-level part of the `AccessTypeRole` row, but never a role name. The role name comes from whichever `HasRole` attaches to it, and `isGroupAccessEnabled()` decides which column it lands in — `GroupRoleName` when true, `OrganisationalRoleName` when false. So one constant can be shared by several roles, and one role commonly carries several constants, because the same logical access type offered to a second organisation profile is a second row.
- **The SDK derives a `RoleToAccessProfiles` row for every attaching role.** Because the definition store performs no referential check on `GroupRoleName`, an unmapped group role otherwise imports cleanly and then grants nothing; deriving the row makes that failure impossible rather than silent. The derived row's access profile defaults to the role's **own name** (`CaseRoleToAccessProfile.CaseRoleToAccessProfileBuilder.builder` seeds `accessProfiles` from `role.getRole()`), which puts you in the second of the two cases described below — the group role gets a profile of its own and therefore needs its own ACLs. To have it inherit an existing profile's permissions instead, declare `caseRoleToAccessProfile(role).accessProfiles(...)` by hand; a row already declared for that role wins over the derived one.
- **`caseAssignedRoleField` is a role name, not a field name.** Despite the column name, it is the value the runtime matches against `OrgPolicyCaseAssignedRole`, typically a bracketed case role such as `[SOLICITOR]`. See `AccessTypeRolesValidator.java:159` and `CaseAccessGroupUtils.findOrganisationPolicyNodeForCaseRole`. Unlike `GroupRoleName` this column *is* checked referentially against `RoleToAccessProfiles.RoleName`, so the config must declare a `caseRoleToAccessProfile` for that case role or the import is rejected.
- **Only `AccessType` rows are deduplicated against explicit builder calls.** An explicitly-configured `accessType(...)` suppresses the derived row for the same `(accessTypeId, organisationProfileId)`, but derived `AccessTypeRole` rows are always appended. Declaring the same role's participation both ways therefore emits it twice.
...
Both roles involved — the one attaching the access group, which becomes either the organisational role or the group role depending on `isGroupAccessEnabled()`, and the case role naming the `OrganisationPolicy` — go in the case's role class (`R extends HasRole`). That class is the only thing scanned to derive access types (`ConfigBuilderImpl` reads `config.getRoleClass().getEnumConstants()`), so a role declared in a separate enum is silently ignored: no `AccessType` row is emitted and there is no diagnostic. Group roles are ordinary organisational roles that happen to be minted per organisation by PRM, so they are not a distinct kind of thing needing a distinct home. Membership also gives the group role its `AuthorisationCaseType` row and brings it under `shutterService()` / `omitHistoryForRoles()`, and it can then be granted on events with the ordinary `grant(Set<Permission>, R...)`.

Whether the group role *needs* `Authorisation*` rows depends on how its `RoleToAccessProfiles` mapping resolves, and this is worth understanding because ACLs are matched on **access profile**, not on role name (`AccessControlService.hasAccessControlList` intersects `AccessProfile::getAccessProfile` against `AccessControlList::getAccessProfile`). A group role mapped to an access profile that already holds ACLs — as in the `BEFTA_MASTER_GROUPACCESS` fixture, where `CaseProfessionalGroupAccess_GA_Role` maps to `caseworker-befta_jurisdiction_1` — inherits that profile's permissions and needs no rows of its own, which is why it appears in zero `Authorisation*` sheets there. A group role mapped to an access profile of its own name needs its own ACLs, and then role-class membership is what supplies them. That is the SDK default, since the derived `RoleToAccessProfiles` row names the role itself as its access profile — so unless you declare the mapping by hand, expect to supply the group role's `Authorisation*` rows.

**Why no member of `CCDAccessGroup` is role-typed.** A role-typed member would point back into the case's role class, which already references the access group, making the two enums **circularly initialised**: the JVM will not re-enter an in-progress `<clinit>`, so a role captured as an enum constructor argument reads `null` in whichever class initialises second. The shipped interface removes the problem rather than working around it — `getCaseAssignedRoleField()` is a plain `String`, and no group role name appears on the interface at all, since the role side of the relationship is expressed by `HasRole.getAccessGroups()`. An implementation can therefore be an ordinary enum with every field set from constructor arguments, with no lazily-resolving method bodies or per-constant overrides. The trade-off is that `caseAssignedRoleField` is checked at import rather than compile time, against `RoleToAccessProfiles.RoleName`.
