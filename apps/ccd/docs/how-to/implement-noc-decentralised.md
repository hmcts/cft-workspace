...
8. **Elasticsearch can go stale after the role change, if your case-list visibility depends on indexed content.** Step 4's `addCaseUserRoles`/`removeCaseUserRoles` calls write only to AMRAS — they never touch the service's own case-data table. This is invisible for services whose search visibility is query-time (a `CASE`-scoped role assignment resolved fresh against the caller's RAS grants at search time), but it matters if a service instead derives a group-access-style field from its own tables and relies on Elasticsearch having indexed that field: the indexed document keeps the pre-NoC value even though a direct `GET` on the case reflects the new representation immediately. Manually queuing the case for re-index is not sufficient to fix this — see [Decentralisation: the queue is trigger-driven; the document body is not](../explanation/decentralisation.md#the-queue-is-trigger-driven-the-document-body-is-not) for why only a recorded event produces a fresh indexed document body, and what PCS's NoC flow had to add to get one.
