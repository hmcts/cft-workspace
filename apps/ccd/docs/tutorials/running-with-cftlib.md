...
| A strict OIDC client (e.g. `openid-client` v6) rejects sign-in against the local stack with "invalid response encountered" or an issuer mismatch | `rse-idam-simulator`'s discovery document advertises its issuer with a `/o` suffix, but the `iss` claim inside the tokens it actually issues omits that suffix. A client library that validates `iss` strictly against the discovered value will reject every token. Reconcile your OIDC client against the issuer the simulator actually puts in its tokens, rather than trusting discovery. |
