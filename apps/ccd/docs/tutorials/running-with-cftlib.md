...
| A definition that imports fine on a preview and on AAT is rejected by `cftlibTest`, with a validation message from the importer | cftlib does not pull a definition-store *image*; it builds one from a git submodule in the `rse-cft-lib` repo, pinned to a commit. So a definition-store fix reaches cftlib only once that submodule is bumped **and** a cftlib release is cut — which is later than the fix reaching any deployed environment, and later than a preview that pins the image directly. The failure is real but is a gap in local tooling, not in the definition: check what `projects/ccd-definition-store-api` is pinned to on the cftlib version you are using before changing the definition to satisfy it. A tag pushed on an rse-cft-lib branch publishes that tag name as a usable plugin version, so an unreleased fix can be consumed without waiting for a merge. |
