...
integrations:
  ...
  - bulk_print
api_specs:
  - apps/prl/prl-cos-api:prl_document_upload.json
repos:
...
- **Notice of Change**: `CaseField/NoticeOfChange/` JSON files define `NOCRequest` and `NoticeOfChangeParties` fields; `prl-cos-api` has a `NoticeOfChangeController` and `NoticeOfChangePartiesService` that call `aac-manage-case-assignment` at `${ACA_SERVICE_API_BASEURL}`.
...
- The `prl_document_upload.json` spec in `platops/cnp-api-docs` was manually authored (not generated by a CI workflow); it describes the citizen document upload endpoint exposed via the APIM gateway at `/prl-document-api`, implemented by `CaseDocumentController` in `prl-cos-api`.
