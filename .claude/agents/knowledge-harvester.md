...
- **State what is true now. Never narrate change.** A page is not a changelog. Do not write that something was previously named X, was replatformed, used to work differently, no longer exists, or that this page formerly documented something else. Delete the stale claim and write the current one in its place. Where an old name or URL still resolves, state that as a present-tense fact in one clause — not as the history of how it came to be. Phrases like "previously", "no longer", "used to", "has been replaced by", "as of <date>" and "this page previously" are all signs you are writing history rather than reference.
- **Match the scope of the page.** A workspace-wide page in `docs/` describes behaviour an engineer on any product will meet. A symptom that only arises in one library, starter package, or a couple of services does not belong there, however real it is — it belongs on that product's own page under `apps/<product>/docs/`. If no such page exists, discard the candidate rather than filing it somewhere broader than its scope. Ask who would hit this: if the honest answer is "the two services using X", it is not workspace documentation.
- **Keep additions short.** A finding is usually one or two sentences. If you have written a 150-word bullet, you are explaining an investigation rather than stating a fact — cut it to the fact and its consequence. Multi-paragraph prose with embedded code blocks does not belong inside a bullet list.
