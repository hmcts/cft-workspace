...
Spawn a phase's subagents in batches of about 5, each batch as a single assistant message with multiple tool uses so the harness parallelises them. Each subagent makes its own tool calls, so a wider fan-out multiplies quickly. Phase-3.5 (Confluence) should self-throttle if the Atlassian MCP returns HTTP 429 — back off and retry that page rather than failing the whole run.
