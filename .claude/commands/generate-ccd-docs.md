---
description: Generate or refresh the comprehensive CCD documentation under apps/ccd/docs/. Phased, resumable, fans out specialised subagents in parallel.
---
...
- `/generate-ccd-docs --phase scaffold` — create the `apps/ccd/docs/` skeleton (stub pages with frontmatter only).
- `/generate-ccd-docs --phase research` — fan out `ccd-source-researcher` subagents to populate `apps/ccd/docs/.work/research/`.
...
- `/generate-ccd-docs --phase link` — build glossary, cross-link pages, write `apps/ccd/docs/README.md`.
...
The skill is **resumable** — re-running picks up where it left off, using `apps/ccd/docs/.work/manifest.yaml` as the source of truth. Subagents always overwrite the page they're assigned to; if you have hand-edits on a drafted page, run with `--rephase synth --page <path>` deliberately.
